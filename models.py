...
'''
model.py
이 파일은 데이터베이스의 제약 조건을 명시하는 파일
관계형 데이터베이스의 데이터를 객체랑 연결 시켜주는 것을 ORM (Object Relational Mapping)
즉, 이 파일은 외부에 존재하는 DB를 서버에서 사용하기 위해, DB와 동일한 제약조건을 객체에 걸어버리기
'''
...
class rabbitMenu(db.Model):
    ...
    store_id    = db.Column(db.Integer, db.ForeignKey('rabbitStore.id')) # 외부키를 사용할 때는 '테이블 이름.속성' 을 사용해야 합니다. 객체 이름이 아님
